...
from model.model import Module, List, Struct, Enumeration, PrimitiveType, Array
...
class CToStringGenerator:
        ...
        def write_to_string_body():
            def write_switch_block():
                for value in enum.values:
                    ...
                    self._module_out.writeln(f'OutputHandler_proces(out, "{value}");')
                    self._module_out.unindent()
                self._module_out.writeln(f'default:')
                self._module_out.indent()
                self._module_out.writeln(f'return "<unknown>";')
                self._module_out.unindent()
    ...
    def _write_struct_to_string(self, struct: Struct):
        ...
        struct_c_signature = f'void {struct_c_name}_to_string({struct_c_name} value, struct OutputHandler *out)'
        ...
        def write_field_to_string_call(field):
            if type(field.type) is Struct:
                self._module_out.writeln(f'{self._struct_c_name(field.type)}_to_string(value.{field.name}, out);')
                ...
                self._module_out.writeln(f'OutputHandler_process(out, "%s: {field_code}", "{field.name}", value.{field.name});')
            elif type(field.type) is Array:
                def write_array_element_to_string():
                    if type(field.type.element_type) is Struct:
                        self._module_out.writeln(f'{self._struct_c_name(field.type.element_type)}_to_string(value.{field.name}[index], out);')
                    elif type(field.type) is Enumeration:
                ...
                self._before_block(f'for (size_t index = 0; index < {field.type.length})')
